...
fun Iterator<IndexedValue<Iota>>.nextList(argc: Int = 0): SpellList {
    val (idx, x) = this.next()
    if (x is ListIota) {
        return x.castTo(ListIota.TYPE).list
    } else {
...
fun Iterator<IndexedValue<Iota>>.nextDouble(argc: Int = 0): Double {
    val (idx, x) = this.next()
    if (x is DoubleIota) return x.castTo(DoubleIota.TYPE).double
...
fun Iterator<IndexedValue<Iota>>.nextInt(argc: Int = 0): Int {
    val (idx, x) = this.next()
    if (x is DoubleIota) {
        val double = x.castTo(DoubleIota.TYPE).double
...
fun <T: Iota> Iota.castTo(iotaType: IotaType<T>): T {
    return (this as IMixinIota).tryCastTo(iotaType).orElseGet {throw IllegalStateException("Attempting to downcast $this to type: $iotaType")}
}
