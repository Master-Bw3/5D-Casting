package net.masterbw3.fivedimcasting.client.gui
...
class GuiGrandStaffSpellCasting constructor(
    private val handOpenedWith: Hand,
    private var patterns: MutableList<ResolvedPattern>,
    private var cachedStack: List<NbtCompound>,
    private var cachedRavenmind: NbtCompound?,
    private var parenCount: Int,
) : Screen("gui.hexcasting.spellcasting".asTranslatedComponent) {
    private var stackDescs: List<OrderedText> = listOf()
    private var parenDescs: List<OrderedText> = listOf()
    private var ravenmind: OrderedText? = null
    ...
    private val randSrc = SoundInstance.createRandom()
    ...
    fun recvServerUpdate(info: ExecutionClientView, index: Int) {
        ...
        if (info.resolutionType == ResolvedPatternType.UNDONE) {
            this.patterns.reversed().drop(1).firstOrNull { it.type == ResolvedPatternType.ESCAPED }
    ...
    fun calculateIotaDisplays() {
        ...
        this.stackDescs =
            this.cachedStack.map { IotaType.getDisplayWithMaxWidth(it, width, mc.textRenderer) }
                .asReversed()
...
//            emptyList()
    ...
    override fun init() {
        ...
        soundManager.stopSounds(HexSounds.CASTING_AMBIANCE.id, null)
    ...
    override fun tick() {
        ...
        val player = minecraft.player
        if (player != null) {
            val heldItem = player.getStackInHand(handOpenedWith)
            if (heldItem.isEmpty || !heldItem.isIn(HexTags.Items.STAVES))
                closeForReal()
        }
    }
...
//                )
    ...
    override fun mouseDragged(mxOut: Double, myOut: Double, pButton: Int, pDragX: Double, pDragY: Double): Boolean {
        ...
        if (anchorCoord != null) {
            ...
            val snapDist =
                this.hexSize() * this.hexSize() * 2.0 * MathHelper.clamp(
                    HexConfig.client().gridSnapThreshold(),
                    0.5,
                    1.0
                )
            if (anchor.distanceSquared(mouse) >= snapDist) {
                val delta = mouse.add(anchor.negate())
                val angle = atan2(delta.y, delta.x)
                // 0 is right, increases clockwise(?)
                val snappedAngle = angle.div(MathHelper.TAU).mod(6.0f)
...
//                    )
    ...
    override fun mouseScrolled(pMouseX: Double, pMouseY: Double, pDelta: Double): Boolean {
        ...
        val mouseHandler = MinecraftClient.getInstance().mouse
    ...
    fun closeForReal() {
        MinecraftClient.getInstance().soundManager.stopSounds(HexSounds.CASTING_AMBIANCE.id, null)
...
//        this.ambianceSoundInstance?.mousePosY = pMouseX / this.width.toDouble()
        ...
        val centerCoord = this.pxToCoord(center)
        val radius = 5
...
//        }
        ...
        if (this.stackDescs.isNotEmpty()) {
            ...
            for (desc in this.stackDescs) {
                graphics.drawTextWithShadow(font, desc, 5, 7, -1) // TODO: Confirm this works
                ps.translate(0.0, 10.0, 0.0)
            }
        }
        ...
        if (this.ravenmind != null) {
            ...
            val opacity = (MathHelper.map(sin(time), -1f, 1f, 150f, 255f)).toInt()
            val color = 0x00_ffffff or (opacity shl 24)

            RenderSystem.setShader { prevShader }
            graphics.drawTextWithShadow(font, kotlinBad, 0, 0, color) // TODO: Confirm this works
            ps.pop()
        }
    ...
    fun hexSize(): Float {
        val scaleModifier = MinecraftClient.getInstance().player!!.getAttributeValue(HexAttributes.GRID_ZOOM)
    ...
    fun coordsOffset(): Vec2f = Vec2f(this.width.toFloat() * 0.5f, this.height.toFloat() * 0.5f)

    fun coordToPx(coord: HexCoord) =
        at.petrak.hexcasting.api.utils.coordToPx(coord, this.hexSize(), this.coordsOffset())

    fun pxToCoord(px: Vec2f) = at.petrak.hexcasting.api.utils.pxToCoord(px, this.hexSize(), this.coordsOffset())
    ...
    companion object {
        ...
        fun drawBox(ps: MatrixStack, x: Float, y: Float, w: Float, h: Float, leftMargin: Float = 2.5f) {
            RenderSystem.setShader(GameRenderer::getPositionColorProgram)
            RenderSystem.enableBlend()
